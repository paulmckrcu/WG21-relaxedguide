...
the current draft.
The term \emph{strict C++} refers to the subset of full C++ obtained by
dropping the following vague ``normative encouragement'' from the C++20
memory model:
``Implementations should ensure that no "out-of-thin-air" values are
computed that circularly depend on their own computation.''
Some (but not all) of the proto-patterns in this document are
safe in strict C++, but all of them are safe in full C++.
...
Note that a thread releasing its last reference is forbidden from
using a relaxed operation to do so, because in that case there is
no guarantee that another reference is guaranteed to be held
throughout.
...
\emph{Note:} This analysis does not take into account the possibility of
circular self-justifying propagation of undefined behavior.
% @@@ Cite David Goldblatt's paper.
...
However, neither OOTA nor RFUN behavior has been observed in practice.
...
\item	\emph{Strict-C++ Safe:} The design pattern is expected to
	work correctly in Strict C++ as well as in Full C++.
	A lower case ``y'' indicates that only some examples in the
	category are agreed to be safe for Strict C++.
\end{enumerate}
